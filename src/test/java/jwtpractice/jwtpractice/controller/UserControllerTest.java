...
import jwtpractice.jwtpractice.domain.UserJoinRequest;
import jwtpractice.jwtpractice.domain.UserLoginRequest;
...
class UserControllerTest {
    ...
    @Autowired
    ObjectMapper objectMapper; //객체->json 잭슨 라이브러리

    @Test
    @DisplayName("회원가입 성공")
    @WithMockUser
    void join() throws Exception {
        String userName = "reheat";
        String password = "1q2w3e4r";

        mockMvc.perform(post("/api/v1/users/join")
                        .with(csrf()) // 스프링시큐리티 호출시 필수 //스프링 시큐리티 테스트 받아야 한다.
